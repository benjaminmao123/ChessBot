...
class Board:
    ...
    def init(self):
        ...
        if self.is_flipped():
            board = self.__driver.find_elements(By.CSS_SELECTOR, strings.board_flipped)[0]

            row_start, row_end, row_increment = 1, 9, 1
            col_start, col_end, col_increment = 8, 0, -1
        else:
            board = self.__driver.find_elements(By.CSS_SELECTOR, strings.board)[0]

            row_start, row_end, row_increment = 8, 0, -1
            col_start, col_end, col_increment = 1, 9, 1

        pieces = self.__driver.find_elements(By.CSS_SELECTOR, strings.pieces)
        pieces_map = {}

        for piece in pieces:
            element_name = piece.get_attribute('class')

            piece_name_start_idx = element_name.find('w') if element_name.find('w') != -1 else element_name.find('b')
            piece_name = element_name[piece_name_start_idx:piece_name_start_idx + 2]

            key_start_idx = element_name.find('-') + 1
            key = Parser.numbers_to_letters[element_name[key_start_idx]] + element_name[key_start_idx + 1]

            pieces_map[key] = (piece, piece_name)

        square_width, square_height = board.size['width'] / 8, board.size['height'] / 8
        ...
        for row_num, row_offset in zip(range(row_start, row_end, row_increment), range(0, 8)):
            for col, col_offset in zip(range(col_start, col_end, col_increment), range(0, 8)):
                ...
                self.__board_mapping[key] = {'x': board.location['x'] + square_width * col_offset,
                                             'y': board.location['y'] + square_height * row_offset,
                                             'element': None,
                                             'piece_name': None}

                if key in pieces_map:
                    self.__board_mapping[key]['element'] = pieces_map[key][0]
                    self.__board_mapping[key]['piece_name'] = pieces_map[key][1]
    ...
    def get_board_element(self, key):
    ...
    def __overwrite_elements(self, move):
        ...
        if move['promotion'][0]:
            ...
            promo_square_num = Parser.letters_to_numbers[dest_square[0]] + dest_square[1]
            promo_element = self.__driver.find_elements(By.CSS_SELECTOR, f'.square-{promo_square_num}')
            self.__board_mapping[dest_square]['element'] = promo_element[0]
            self.__board_mapping[dest_square]['piece'] = move['promotion'][1]
    ...
    def __overwrite_element(self, src, dest):
        ...
        src_element = self.__board_mapping[src]['element']
        ...
        self.__board_mapping[src]['element'] = None
        self.__board_mapping[src]['piece_name'] = None
        self.__board_mapping[dest]['element'] = src_element
